...
class SelfAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.query = nn.Conv1d(in_channels, in_channels // 8, kernel_size=4)
        self.key = nn.Conv1d(in_channels, in_channels // 8, kernel_size=4)
        self.value = nn.Conv1d(in_channels, in_channels, kernel_size=4)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=2):
        ...
        self.block = nn.Sequential(
            nn.ConvTranspose1d(in_channels, out_channels, kernel_size=4, stride=stride, padding=1, output_padding=stride-1),
            nn.BatchNorm1d(out_channels),
            nn.ReLU(True),
            nn.ConvTranspose1d(out_channels, out_channels, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm1d(out_channels)
        )
        self.residual = nn.ConvTranspose1d(in_channels, out_channels, kernel_size=4, stride=stride, padding=0, output_padding=stride-1)
...
class Generator(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.gen = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Upsample(4000, mode='linear', align_corners=False) # Upsample to 4000
        )
